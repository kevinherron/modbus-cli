...
public record OutputOptions(
    OutputFormat format, boolean verbose, boolean colorsEnabled, EmitMode emitMode) {

  /** Default output options: human format, normal verbosity, colors enabled, emit all. */
  public static OutputOptions defaults() {
    return new OutputOptions(OutputFormat.HUMAN, false, true, EmitMode.ALL);
  }
  ...
  public static OutputOptions human(boolean verbose, boolean colorsEnabled) {
    return new OutputOptions(OutputFormat.HUMAN, verbose, colorsEnabled, EmitMode.ALL);
  }

  /** Creates options for JSON output. */
  public static OutputOptions json() {
    return new OutputOptions(OutputFormat.JSON, false, false, EmitMode.ALL);
  }
}
