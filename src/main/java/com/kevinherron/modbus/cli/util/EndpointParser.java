...
public final class EndpointParser {
   ...
   *
   * <p>The {@code portOverride} parameter corresponds to the {@code --port} CLI flag. When
   * provided, it overrides the default TCP port. If both the endpoint string and {@code
   * portOverride} specify a port and they disagree, an {@link IllegalArgumentException} is thrown.
   *
   * @param rawEndpoint the raw endpoint string to parse.
   * @param portOverride an optional port override from the {@code --port} CLI flag.
   * @return the resolved endpoint.
   * @throws IllegalArgumentException if the endpoint is invalid or has conflicting port values.
   */
  ...
  private static ParsedTcp parseTcpUri(String endpoint) {
    ...
    @Nullable Integer endpointPort = uri.getPort() >= 0 ? uri.getPort() : null;
  ...
  /**
   * Intermediate result from parsing a TCP endpoint string, before port resolution. The port may be
