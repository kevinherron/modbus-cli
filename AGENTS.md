...
- Use primitive types in patterns and switches (JDK 25+)
  ```java
...
- Use Scoped Values (JDK 25+) instead of `ThreadLocal` for sharing immutable data across threads
  ```java
  // Scoped values are immutable and more efficient than ThreadLocal
  final static ScopedValue<User> CURRENT_USER = ScopedValue.newInstance();

  ScopedValue.where(CURRENT_USER, user).run(() -> {
      // CURRENT_USER is available here and in any called methods
      processRequest();
  });
  ```

- Consider Structured Concurrency (JDK 25+) for managing concurrent tasks
  ```java
...
- Javadoc tag descriptions must begin with lowercase letters (except for proper/class names) and end
  with a period.
  ```java
  /**
...
- Be aware that JDK 25 includes compact object headers and JIT compiler improvements that can
  significantly reduce memory overhead and improve startup time

- Consider using JDK Flight Recorder (JFR) features for profiling:
    - CPU-time profiling for performance analysis
    - Method timing and tracing to identify bottlenecks
    - Cooperative sampling for more stable thread stack analysis
...
After completing any development cycle, ALWAYS run:

```bash
mvn clean verify
```

This will compile, format check, and run all tests to ensure the build is successful.
